...
/*--IMPRIMIR CERTIFICADO*/
    $(document).off( 'click', '#CertBtnQuimi' );
    $(document).on( 'click', '#CertBtnQuimi', function(event){
        var vLinha = $( this ).parent().parent();
        Core.LoadMenu.show()
        Core.Quimico.SetImpreCertQuimi( { 
            evento:{
                sist_para_tenant: Core.Quimico.GetDataTableQuimi( '#TableQuimi', vLinha, 'amos_cada_tenan' ),
				sist_para_empre: Core.Quimico.GetDataTableQuimi( '#TableQuimi', vLinha, 'amos_cada_empre' ), 
                amos_cada_iden: Core.Quimico.GetDataTableQuimi( '#TableQuimi', vLinha, 'amos_cada_iden' ),
                amos_cada_cert: 'QUIMI', 
            }
        }, function( vRespLogi ){
            if ( vRespLogi.status != 'sucesso' ){
                $( '#AlertMenuCont' ).html(
