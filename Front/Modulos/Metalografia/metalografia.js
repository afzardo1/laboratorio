...
/*--IMPRIMIR CERTIFICADO*/
    $(document).off( 'click', '#CertBtnMeta' );
    $(document).on( 'click', '#CertBtnMeta', function(event){
        var vLinha = $( this ).parent().parent();
        Core.LoadMenu.show()
        Core.Metalografia.SetImpreCertMeta( { 
            evento:{
                sist_para_tenant: Core.Metalografia.GetDataTableMeta( '#TableMeta', vLinha, 'amos_cada_tenan' ),
				sist_para_empre: Core.Metalografia.GetDataTableMeta( '#TableMeta', vLinha, 'amos_cada_empre' ), 
                amos_cada_iden: Core.Metalografia.GetDataTableMeta( '#TableMeta', vLinha, 'amos_cada_iden' ),
                amos_cada_cert: 'METALO', 
            }
        }, function( vRespLogi ){
            if ( vRespLogi.status != 'sucesso' ){
                $( '#AlertMenuCont' ).html(
