<?php
	namespace Back\Modulos\Metalografia;
		...
		public static function SetImpreMeta( $Parametros = array() ){
			...
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
				...
				try{
					...
					$GetRegMeta = str_replace(
						':FILTRO',
						'( amos_meta_cada_iden LIKE :amos_meta_cada_iden )', 
						self::$RotSql[ 'GetRegMeta' ]
					);
					...
					$listreg = Core::SetGeraPdf(
						$Cabecalho,
						$Corpo,
						str_replace( array( 'Back\Modulos\Metalografia', 'Back/Modulos/Metalografia' ), '', __DIR__ ).'Imagem/'.$Retorno[0]['sis_para_logo'],
						'P',
						'SIMPLES',
						array( 10, 40, 10, 5 )
					);
