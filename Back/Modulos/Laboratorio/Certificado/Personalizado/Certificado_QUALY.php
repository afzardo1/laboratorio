<?php
	namespace Back\Modulos\Laboratorio\Certificado\Personalizado;
	...
	/**
 	 * Esta classe contendo rotinas certificados personalizado tenant QUALYSERVICE
 	 * Todas as requisições certificados passam por aqui, são tratadas
 	 * e redirecionadas para o seu destino.
 	 *
 	 * @package    Certificado_QUALY
 	 * @author     Alexandre Farinelli Zardo
 	*/
	final class Certificado_QUALY {
		/**
		...
		*/
		public static function GetCertAchaExpa( $Parametros = array() ){
			...
			$AnexCami = str_replace( array( 'Back\Modulos\Laboratorio\Certificado', 'Back/Modulos/Laboratorio/Certificado' ), '', __DIR__ ).'Anexos/ACHATAMENTO_'.$Retorno[ 'listreg' ][0]['amos_achat_cada_iden'].'_';
		...
		*/
		public static function GetCertMeta( $Parametros = array() ){
			...
			$AnexCami = str_replace( array( 'Back\Modulos\Laboratorio\Certificado', 'Back/Modulos/Laboratorio/Certificado' ), '', __DIR__ ).'Anexos/METALOGRAFIA_'.$Retorno[ 'listreg' ][0]['amos_meta_cada_iden'].'_';
		...
		public static function SetImpreCert( $Parametros = array() ){
			...
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
				...
				try{
					...
					Core::$CabeSelo = '';
					if( $Cabecalho[ 'selo' ] != '' ){
						Core::$CabeSelo = str_replace( array( 'Back\Modulos\Laboratorio\Certificado', 'Back/Modulos/Laboratorio/Certificado' ), '', __DIR__ ).'Imagem/'.$Cabecalho[ 'selo' ];
					};

					$CabeLogo = '';
					if( $Cabecalho[ 'logo' ] != '' ){
						$CabeLogo = str_replace( array( 'Back\Modulos\Laboratorio\Certificado', 'Back/Modulos/Laboratorio/Certificado' ), '', __DIR__ ).'Imagem/'.$Cabecalho[ 'logo' ];
					};
