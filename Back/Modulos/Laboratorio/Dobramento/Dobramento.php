<?php
	namespace Back\Modulos\Laboratorio\Dobramento;
	...
	/**
 	 * Esta classe contendo rotinas Dobramento
 	 * Todas as requisições Dobramento passam por aqui, são tratadas
 	 * e redirecionadas para o seu destino.
 	 *
 	 * @package    Dobramento
 	 * @author     Alexandre Farinelli Zardo
 	*/
	final class Dobramento {
		/**
		...
		public static function Inicia(){
			include_once( 'Dobramento.sql.php' );

			self::$RotSql[ 'GetRegDobra' ] = $GetRegDobra;
			
			self::$RotSql[ 'InstRegDobra' ] = $InstRegDobra;
			self::$RotSql[ 'UpdtRegDobra' ] = $UpdtRegDobra;
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetTenanDobra( $Parametros = array() ){
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetEmpreDobra( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetClieDobra( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetFabrDobra( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetMateDobra( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetAreaDobra( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		 public static function GetUsuaDobra( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetDobra( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
				Dobramento::Inicia();
				...
				try {
					...
					$GetRegDobra = str_replace(
						...
						'( amos_cada_regi LIKE :amos_cada_regi ) AND
						 ...
						 ( amos_dobra_cada_fina LIKE :amos_dobra_cada_fina OR
						   amos_dobra_cada_fina IS NULL )',
						self::$RotSql[ 'GetRegDobra' ]
					);

					$Prepara = self::$Conn->prepare( $GetRegDobra );
					...
					$Prepara->bindValue( ':amos_dobra_cada_fina', $Parametros[ 'amos_dobra_cada_fina' ] );
					...
					$Retorno = array_map( function( $input ) {
						$Botao = '';
						$Figura = 'fas fa-eye';
						if ( isset( $input['amos_dobra_cada_boto'] ) == false){
							
							if ( $input['amos_dobra_cada_fina'] == 0 ){
								$Figura = 'fas fa-edit';
							};
							
							$Botao = $Botao.'
								<button id="EditBtnDobra" type="button" class="btn btn-secondary" title="ALTERAR"><i class="'.$Figura.'"></i></button>
								<button id="ImprBtnDobra" type="button" class="btn btn-success" title="IMPRIMIR"><i class="fas fa-print"></i></button>
							';
						};
						
						return array(
							'amos_dobra_cada_boto' => $Botao,
							...
							'amos_dobra_cada_iden' => $input['amos_dobra_cada_iden'],
							'amos_dobra_cada_qtde' => $input['amos_dobra_cada_qtde'],
							'amos_dobra_cada_result' => $input['amos_dobra_cada_result'],
							'amos_dobra_cada_obs' => $input['amos_dobra_cada_obs'],
							'amos_dobra_cada_fina' => $input['amos_dobra_cada_fina'],
							'amos_dobra_cada_fina_data' => $input['amos_dobra_cada_fina_data'],
							'amos_dobra_cada_fina_usua_iden' => $input['amos_dobra_cada_fina_usua_iden'],
						);
					}, $Retorno );
					...
					return json_encode( array(
						...
						'modulo' => 'Dobramento',
						'status' => 'sucesso',
						'descricao' => 'Resultado Pesquisa Dobramento',
						'listreg' => $Retorno,
					));
					...
					return json_encode( array(
						...
						'modulo' => 'Dobramento',
						'status' => 'invalido',
						'descricao' => 'Pesquisa Dobramento com erro </br> '.$e->getMessage(),
						'listreg' => false,
					));
				};
			} else {
		 ...
		 * @access public
		*/
		public static function SetSalvDobra( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
				$ParamVali = array(
				...
				try {
					...
					if ( $Parametros[ 'IdenDobra' ] == 'Automatico' ){
						$Parametros[ 'IdenDobra' ] = 0;
					};

					self::$Conn->beginTransaction();

					if ( $Parametros[ 'IdenDobra' ] == 0 ){
						$descricao = 'Inclusão de Dobramento';
						$Prepara = self::$Conn->prepare( self::$RotSql[ 'InstRegDobra' ] );
					} else {
						$descricao = 'Alteração de Dobramento';
						$Prepara = self::$Conn->prepare( self::$RotSql[ 'UpdtRegDobra' ] );
						$Prepara->bindValue( ':amos_dobra_cada_iden', $Parametros[ 'IdenDobra' ] );
					};

					$Prepara->bindValue( ':amos_dobra_cada_amos_iden', $Parametros[ 'IdenAmos' ] );
					$Prepara->bindValue( ':amos_dobra_cada_qtde', str_replace( '.', '', $Parametros[ 'QtdeDobra' ] ) );
					$Prepara->bindValue( ':amos_dobra_cada_result', $Parametros[ 'ResulDobra' ] );
					$Prepara->bindValue( ':amos_dobra_cada_obs',  Core::UpperCase( $Parametros[ 'ObseDobra' ] ) );
					$Prepara->bindValue( ':amos_dobra_cada_fina', str_replace( array( 'false', 'true' ), array( '0', '1' ), $Parametros[ 'FinaDobra' ] ) );

					if ( $Parametros[ 'FinaDobra' ] == 'true' ){
						$Prepara->bindValue( ':amos_dobra_cada_fina_data', Core::GetDataMariaDb( $Parametros[ 'DatFinaDobra' ] ) );
						$Prepara->bindValue( ':amos_dobra_cada_fina_usua_iden', $Parametros[ 'ExcutDobra' ] );
					} else {
						$Prepara->bindValue( ':amos_dobra_cada_fina_data', 'NULL' );
						$Prepara->bindValue( ':amos_dobra_cada_fina_usua_iden', 'NULL' );
					}
					$Prepara->execute();
					
					if ( $descricao == 'Inclusão de Dobramento' ){
						$Retorno = self::$Conn->lastInsertId().' incluido';
						$Parametros[ 'IdenDobra' ] = self::$Conn->lastInsertId();
					} else {
						$Retorno = $Parametros[ 'IdenDobra' ].' alterado';
					};


					$RetorAnexos = Anexos::SetSalvAnex( $Parametros[ 'AnexosDobra' ], $Parametros[ 'anexo_ensa_tabe' ], $Parametros[ 'IdenDobra' ], self::$Conn );
					...
					return json_encode( array(
						...
						'modulo' => 'Dobramento',
						'status' => 'invalido',
						'descricao' => 'Manutenção de Dobramento com erro </br> '.$e->getMessage(),
						'listreg' => false,
					));
				};
			} else {
		 ...
		 * @access public
		*/
		public static function SetImpreDobra( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
				Dobramento::Inicia();
				...
				try{
					...
					$Prepara->bindValue( ':amos_cada_iden', $Parametros[ 'IdenDobra' ] );
						...
						<table border="0" style="border-collapse: collapse">
        					<tr>
	          					...
	          					<td width="580" align="center">
    	        					<span style="font-size: 15pt"><b> DOBRAMENTO </b></span>
        	  					</td>
              				</tr>
							<tr>
          						<td width="681">
					...
					if ( $Retorno[0]['amos_dobra_cada_result'] != -1 ){
						$amos_dobra_cada_result = str_replace(
							array( 0, 1, 2, 3 ), 
							array( 'REPROVADO', 'INFORMATIVO', 'TOLERAVEL', 'APROVADO' ),
							$Retorno[0]['amos_dobra_cada_result']
						);
					};

					$amos_dobra_cada_fina_data = '';
					if ( $Retorno[0]['amos_dobra_cada_fina_data'] != '' && $Retorno[0]['amos_dobra_cada_fina_data'] != '0000-00-00 00:00:00' ){
						$amos_dobra_cada_fina_data = date( Core::config( 'date_format' ), strtotime( $Retorno[0]['amos_dobra_cada_fina_data'] ) );
					};
					...
					$Corpo = '
						...
						<table border="1" cellpadding="1" style="border-collapse: collapse; font-size: 10pt">		
							<tr>
								<td width="264"> <b>Tipo Dobramento: </b>'.$Retorno[0]['amos_cada_tipo'].'</td>
								<td width="190"> <b>Quantidade: </b>'.$Retorno[0]['amos_dobra_cada_qtde'].'</td>
								<td width="226" align="center" style="background-color:#d5d6d8;"> <b>Dobramento: </b>'.$amos_dobra_cada_result.'</td>
							</tr>
						</table>
						<br>
						<br>
						...
						<table border="1" cellpadding="1" style="border-collapse: collapse; font-size: 9pt">
							<tr>
								<td width="680" style="height:97px">
									 <b>Observação</b>
									<br>
									 '.nl2br( $Retorno[0]['amos_dobra_cada_obs'] ).'
								</td>
							</tr>
						</table>
						<br>
						...
						<table border="1" cellpadding="1" style="border-collapse: collapse; font-size: 10pt">
							<tr>
								<td width="125"> <b>Finalizado ?: </b>'.str_replace( array( 0, 1 ), array( 'NÃO', 'SIM' ), $Retorno[0]['amos_dobra_cada_fina'] ).'</td>
								<td width="180"> <b>Finalizado Em: </b>'.$amos_dobra_cada_fina_data.'</td>
					...
					if( $Retorno[0]['sis_para_logo'] != '' ){
						$CabeLogo = str_replace( array( 'Back\Modulos\Laboratorio\Dobramento', 'Back/Modulos/Laboratorio/Dobramento' ), '', __DIR__ ).'Imagem/'.$Retorno[0]['sis_para_logo'];
					};
					...
					return json_encode( array(
						...
						'modulo' => 'Dobramento',
						'status' => 'sucesso',
						'descricao' => 'Impressão de Dobramento',
						'listreg' => $listreg,
					));
				} catch ( PDOException $e ) {
					return json_encode( array(
						...
						'modulo' => 'Dobramento',
						'status' => 'invalido',
						'descricao' => 'Impressão de Dobramento com erro </br> '.$e->getMessage(),
						'listreg' => false,
					));
				};
			} else {
