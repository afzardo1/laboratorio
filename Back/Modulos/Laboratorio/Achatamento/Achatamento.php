<?php
	namespace Back\Modulos\Laboratorio\Achatamento;
	...
	/**
 	 * Esta classe contendo rotinas Achatamento / Expansão
 	 * Todas as requisições Achatamento / Expansão passam por aqui, são tratadas
 	 * e redirecionadas para o seu destino.
 	 *
 	 * @package    AchatExpan
 	 * @author     Alexandre Farinelli Zardo
 	*/
	final class Achatamento {
		/**
		...
		public static function Inicia(){
			include_once( 'Achatamento.sql.php' );

			self::$RotSql[ 'GetRegAchat' ] = $GetRegAchat;
			
			self::$RotSql[ 'InstRegAchat' ] = $InstRegAchat;
			self::$RotSql[ 'UpdtRegAchat' ] = $UpdtRegAchat;
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetTenanAchat( $Parametros = array() ){
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetEmpreAchat( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetClieAchat( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetFabrAchat( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetMateAchat( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetAreaAchat( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		 public static function GetUsuaAchat( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
		 ...
		 * @return mixed
	 	 * @access public
	 	*/
		public static function GetAchat( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
				Achatamento::Inicia();
				...
				try {
					...
					$GetRegAchat = str_replace(
						...
						'( amos_cada_regi LIKE :amos_cada_regi ) AND
						 ...
						 ( amos_achat_cada_fina LIKE :amos_achat_cada_fina OR
						   amos_achat_cada_fina IS NULL )',
					...
					$Prepara->bindValue( ':amos_achat_cada_fina', $Parametros[ 'amos_achat_cada_fina' ] );
					...
					$Retorno = array_map( function( $input ) {
						$Botao = '';
						$Figura = 'fas fa-eye';
						if ( isset( $input['amos_achat_cada_boto'] ) == false){
							
							if ( $input['amos_achat_cada_fina'] == 0 ){
								$Figura = 'fas fa-edit';
							};
						...
						return array(
							...
							'amos_achat_cada_iden' => $input['amos_achat_cada_iden'],
							'amos_achat_cada_qtde' => $input['amos_achat_cada_qtde'],
							'amos_achat_cada_achat_result' => $input['amos_achat_cada_achat_result'],
							'amos_achat_cada_expan_result' => $input['amos_achat_cada_expan_result'],
							'amos_achat_cada_obs' => $input['amos_achat_cada_obs'],
							'amos_achat_cada_fina' => $input['amos_achat_cada_fina'],
							'amos_achat_cada_fina_data' => $input['amos_achat_cada_fina_data'],
							'amos_achat_cada_fina_usua_iden' => $input['amos_achat_cada_fina_usua_iden'],
						);
					}, $Retorno );
					...
					return json_encode( array(
						...
						'status' => 'sucesso',
						'descricao' => 'Resultado Pesquisa Achatamento / Expansão',
						'listreg' => $Retorno,
					));
					...
					return json_encode( array(
						...
						'modulo' => 'Achatamento / Expansao',
						'status' => 'invalido',
						'descricao' => 'Pesquisa Achatamento / Expansão com erro </br> '.$e->getMessage(),
						'listreg' => false,
					));
				};
			} else {
		 ...
		 * @access public
		*/
		public static function SetSalvAchat( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
				$ParamVali = array(
				...
				if ( ( $PermiInclu[ 'status' ] == 'restrito' ) && ( $Parametros[ 'IdenAchat' ] == 'Automatico' ) ){
					return json_encode( $PermiInclu	);
				};
				
				if ( ( $PermiAlter[ 'status' ] == 'restrito' ) && ( $Parametros[ 'IdenAchat' ] != 'Automatico' ) ){
				...
				try {
					...
					if ( $Parametros[ 'IdenAchat' ] == 'Automatico' ){
						$Parametros[ 'IdenAchat' ] = 0;
					};

					self::$Conn->beginTransaction();

					if ( $Parametros[ 'IdenAchat' ] == 0 ){
						$descricao = 'Inclusão de Achatamento / Expansão';
						$Prepara = self::$Conn->prepare( self::$RotSql[ 'InstRegAchat' ] );
					} else {
						$descricao = 'Alteração de Achatamento / Expansão';
						$Prepara = self::$Conn->prepare( self::$RotSql[ 'UpdtRegAchat' ] );
						$Prepara->bindValue( ':amos_achat_cada_iden', $Parametros[ 'IdenAchat' ] );
					};

					$Prepara->bindValue( ':amos_achat_cada_amos_iden', $Parametros[ 'IdenAmos' ] );
					$Prepara->bindValue( ':amos_achat_cada_qtde', str_replace( '.', '', $Parametros[ 'QtdeAchat' ] ) );
					$Prepara->bindValue( ':amos_achat_cada_achat_result', $Parametros[ 'ResulAchat' ] );
					$Prepara->bindValue( ':amos_achat_cada_expan_result', $Parametros[ 'ResulExpanAchat' ] );
					$Prepara->bindValue( ':amos_achat_cada_obs',  Core::UpperCase( $Parametros[ 'ObseAchat' ] ) );
					$Prepara->bindValue( ':amos_achat_cada_fina', str_replace( array( 'false', 'true' ), array( '0', '1' ), $Parametros[ 'FinaAchat' ] ) );

					if ( $Parametros[ 'FinaAchat' ] == 'true' ){
						$Prepara->bindValue( ':amos_achat_cada_fina_data', Core::GetDataMariaDb( $Parametros[ 'DatFinaAchat' ] ) );
						$Prepara->bindValue( ':amos_achat_cada_fina_usua_iden', $Parametros[ 'ExcutAchat' ] );
					} else {
						$Prepara->bindValue( ':amos_achat_cada_fina_data', 'NULL' );
						$Prepara->bindValue( ':amos_achat_cada_fina_usua_iden', 'NULL' );
					}
					$Prepara->execute();
					
					if ( $descricao == 'Inclusão de Achatamento / Expansão' ){
						$Retorno = self::$Conn->lastInsertId().' incluido';
						$Parametros[ 'IdenAchat' ] = self::$Conn->lastInsertId();
					} else {
						$Retorno = $Parametros[ 'IdenAchat' ].' alterado';
					};

					$RetorAnexos = Anexos::SetSalvAnex( $Parametros[ 'AnexosAchat' ], $Parametros[ 'anexo_ensa_tabe' ], $Parametros[ 'IdenAchat' ], self::$Conn );
					...
					return json_encode( array(
						...
						'modulo' => 'Achatamento / Expansão',
						'status' => 'invalido',
						'descricao' => 'Manutenção de Achatamento / Expansão com erro </br> '.$e->getMessage(),
						'listreg' => false,
					));
				};
			} else {
		 ...
		 * @access public
		*/
		public static function SetImpreAchat( $Parametros = array() ){
			$vStatSess = json_decode( Core::Sessao()::Chk( 'usua_cada_iden' ), true );
			if ( $vStatSess[ 'status' ] == 'aberto' ) {
				Achatamento::Inicia();
				...
				try{
					...
					$Prepara->bindValue( ':amos_cada_iden', $Parametros[ 'IdenAchat' ] );
						...
						<table border="0" style="border-collapse: collapse">
        					<tr>
	          					...
	          					<td width="580" align="center">
    	        					<span style="font-size: 15pt"><b> ENSAIO ACHATAMENTO / ESPANSÃO </b></span>
        	  					</td>
              				</tr>
							<tr>
          						<td width="681">
					...
					if ( $Retorno[0]['amos_achat_cada_achat_result'] != -1 ){
						$amos_achat_cada_achat_result = str_replace(
							array( 0, 1, 2, 3 ), 
							array( 'REPROVADO', 'INFORMATIVO', 'TOLERAVEL', 'APROVADO' ),
							$Retorno[0]['amos_achat_cada_achat_result']
						);
					};

					$amos_achat_cada_expan_result = '';

					if ( $Retorno[0]['amos_achat_cada_expan_result'] != -1 ){
						$amos_achat_cada_expan_result = str_replace(
							array( 0, 1, 2, 3 ), 
							array( 'REPROVADO', 'INFORMATIVO', 'TOLERAVEL', 'APROVADO' ),
							$Retorno[0]['amos_achat_cada_expan_result']
						);
					};

					$amos_achat_cada_fina_data = '';
					if ( $Retorno[0]['amos_achat_cada_fina_data'] != '' && $Retorno[0]['amos_achat_cada_fina_data'] != '0000-00-00 00:00:00' ){
						$amos_achat_cada_fina_data = date( Core::config( 'date_format' ), strtotime( $Retorno[0]['amos_achat_cada_fina_data'] ) );
					};
					...
					$Corpo = '
						...
						<table border="1" cellpadding="1" style="border-collapse: collapse; font-size: 10pt">		
							<tr>
								<td width="226"> <b>Quantidade: </b>'.$Retorno[0]['amos_achat_cada_qtde'].'</td>
								<td width="226" align="center" style="background-color:#d5d6d8;"> <b>Achatamento: </b>'.$amos_achat_cada_achat_result.'</td>
								<td width="227" align="center" style="background-color:#d5d6d8;"> <b>Expansão: </b>'.$amos_achat_cada_expan_result.'</td>
							</tr>
						</table>
						<br>
						<br>
						...
						<table border="1" cellpadding="1" style="border-collapse: collapse; font-size: 9pt">
							<tr>
								<td width="680" style="height:97px">
									 <b>Observação</b>
									<br>
									 '.nl2br( $Retorno[0]['amos_achat_cada_obs'] ).'
								</td>
							</tr>
						</table>
						<br>
						...
						<table border="1" cellpadding="1" style="border-collapse: collapse; font-size: 10pt">
							<tr>
								<td width="125"> <b>Finalizado ?: </b>'.str_replace( array( 0, 1 ), array( 'NÃO', 'SIM' ), $Retorno[0]['amos_achat_cada_fina'] ).'</td>
								<td width="180"> <b>Finalizado Em: </b>'.$amos_achat_cada_fina_data.'</td>
					...
					return json_encode( array(
						...
						'modulo' => 'Achatamento / Expansão',
						'status' => 'sucesso',
						'descricao' => 'Impressão de Achatamento / Expansão',
						'listreg' => $listreg,
					));
				} catch ( PDOException $e ) {
					return json_encode( array(
						...
						'modulo' => 'Achatamento / Expansão',
						'status' => 'invalido',
						'descricao' => 'Impressão de Achatamento / Expansão com erro </br> '.$e->getMessage(),
						'listreg' => false,
					));
				};
			} else {
